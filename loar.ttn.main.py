...
def join_lora(force_join = False):
    ...
    if not lora.has_joined() or force_join == True:
        ...
        dev_eui = binascii.unhexlify(THE_DEV_EUI)
        app_eui = binascii.unhexlify(THE_APP_EUI) # these settings can be found from TTN
        app_key = binascii.unhexlify(THE_APP_KEY) # these settings can be found from TTN
